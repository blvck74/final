...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['message'], $_POST['chat_id']) && !empty(trim($_POST['message']))) {
        ...
        try {
            ...
            // Настройки AI API
            $ai_api_key = 'sk-gfEzUwwBc2ybxPeg56Ac07Ac371c495eB7E52f09883e32F3';  // Ваш действительный API-ключ
            $ai_url = 'https://neuroapi.host/v1/chat/completions';  // Ваш endpoint AI API

            // Формируем данные для отправки к AI API
            // Добавляем системное сообщение, в котором задаём модель как "NeoBrainAI"
            $data = [
                'model' => 'gpt-4o',  // Замените на нужную модель
                'messages' => [
                    [
                        'role' => 'system',
                        'content' => "Ты - ИИ-модель под названием 'FixIt', разработанная для проекта 'FixIt AI'. Всегда представляйся как FixItAI, готовый помочь."
                    ],
                    [
                        'role' => 'user',
